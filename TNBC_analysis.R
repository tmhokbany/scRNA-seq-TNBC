...
packages_needed <- c("Seurat", "ggplot2", "dplyr", "pheatmap", "viridis")
...
cluster_ranking$cluster_name <- TNBC.cluster.ids[as.character(cluster_ranking$seurat_clusters)]
...
# Note: CAFs-vs-immune correlation is already computed and plotted above as
# "9_Correlation_Immune_vs_CAFs.png" (cor_test_immune_caf) — a separate
# flipped-axes version added no new information, so it isn't recomputed here.
cat("\n=== CAF vs Immune Correlation ===\n")
if(exists("cor_test_immune_caf")) {
    cat("r =", round(cor_test_immune_caf$estimate, 3), "\n")
    cat("p-value =", format(cor_test_immune_caf$p.value, scientific = TRUE), "\n")
}
...
if(exists("cor_test_immune_caf")) {
    cat("   - CAFs vs Immune Cells: r =", round(cor_test_immune_caf$estimate, 3),
        " (p =", format(cor_test_immune_caf$p.value, digits = 3), ")\n\n")
}
...
cat("   - Correlation plots: 8_Correlation_CAFs_vs_AP_Suppression.png, 9_Correlation_Immune_vs_CAFs.png\n")
